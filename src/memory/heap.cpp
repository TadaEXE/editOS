...
void reset() noexcept { heap_offset = 0; }
...
void operator delete(void *ptr) noexcept { mem::free(ptr); }

void operator delete[](void *ptr) noexcept { mem::free(ptr); }

// Some ABIs also call sized delete; make them aliases.
void operator delete(void *ptr, size_t) noexcept { mem::free(ptr); }
